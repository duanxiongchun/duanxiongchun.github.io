...
### 2. 多乘员脚手架对齐与题目内容解耦 (Scaffolding Alignment & Content Decoupling)
为了保障系统的高内聚、低耦合与高复用性，系统底座确立了“底座完全对齐，内容按需解耦”的多乘员设计规范：
* **脚手架底座 100% 对齐**：果果（Dabao，6岁）与淼淼（Erbao，2岁）共享完全一致的 **8 大核心特训轨道**、50关递进阶梯进度、连续学习天数翻倍计算以及一键开启的“400关宇宙脑力综合航线”。两个空间的 HUD 控制台架构、兑换商城和发奖流等系统层完全统一。
* **题目内容按龄解耦 (Player Routing)**：通过全局 `window.currentPlayerId`（`'dabao'` 或 `'erbao'`）动态区分玩家：
  * **果果舱（Dabao）**：题库分发调用 WISC-V 与八素高阶认知逻辑题目，配合 3D 动画支架；
  * **淼淼舱（Erbao）**：题库分发调用蒙特梭利早教感官匹配、大颗粒图形、听觉定位和极简 ABAB 规律等适合 2 岁幼儿的特制题目。
  * 这种设计使得我们可以用同一套大厅和转场代码，向不同年龄段的孩子展现最精准的脑力挑战。
